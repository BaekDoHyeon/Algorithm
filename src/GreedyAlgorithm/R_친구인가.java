...
=> 서로소 집합.
==> Union 함수와 Find 함수를 만들어야 하는 알고리즘.. 어느정도는 외워야 한다고 합니다. 그냥 외우는 것도 하나의 방법..
...
public class R_친구인가 {
    ...
    public static int Find(int a) {
        if (a == students[a]) return a;
        else return students[a] = Find(students[a]);
    }

    public static void Union(int a, int b) {
        int av = Find(a);
        int bv = Find(b);
        if (av != bv) students[av] = students[bv];
    }
}
